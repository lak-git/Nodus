  ...
  useEffect(() => {
    if (selectedIncident && selectedIncident.location) {
      map.flyTo(
        ...
        13, // Maintain standard zoom or zoom in closer? Keeping 13 as per request defaults or slightly closer for focus
        {
          animate: true,
          duration: 1.5
        }
...
export function MapView({
  ...
  const getSeverityStyles = (severity: number) => {
    switch (severity) {
      case 5:
        return { color: "#dc2626", radius: 12 }; // Critical - Red
      case 4:
        return { color: "#f97316", radius: 10 }; // High - Orange
      case 3:
        return { color: "#eab308", radius: 8 };  // Medium - Yellow
      case 2:
        return { color: "#3b82f6", radius: 6 };  // Low - Blue
      case 1:
      default:
        return { color: "#22c55e", radius: 6 };  // Minimal - Green
    }
  };
  ...
  return (
    <div className="relative w-full h-full bg-gray-100 rounded-lg overflow-hidden border border-border">
      ...
      {/* Map title overlay */}
      <div className="absolute top-4 left-4 z-[400] bg-white/90 backdrop-blur-sm rounded-lg shadow-md px-4 py-2 pointer-events-none">
      ...
      <div className="absolute top-4 right-4 z-[400] bg-white/90 backdrop-blur-sm rounded-lg shadow-md p-3">
        <div className="text-xs text-[#6B4423] mb-2 font-medium">Severity Legend</div>
